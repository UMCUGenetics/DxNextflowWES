...
/hpc/diaggen/software/tools/nextflow run $workflow_path/WES_preprocess.nf \
-c $workflow_path/WES_preprocess.config \
--fastq_path $input \
--outdir $output \
--email $email \
-profile slurm \
...
if [ \$? -eq 0 ]; then
    ...
    echo "WES preprocess workflow completed successfully."
    rm workflow.running
    touch workflow.done
